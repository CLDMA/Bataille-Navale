...
class ApplicationBatailleNavale:
    def __init__(self, root):
        self.root = root
        self.root.title("Bataille Navale : Nouvelle Version")
    ...
    def on_case_ordinateur_click(self, i, j):
        ...
        if nav_coule:
            for (lx, ly) in nav_coule.positions:
                self.grille_boutons_ordinateur[lx][ly].config(bg="purple", text="X")  # Couleur violet pour les navires coulés
    ...
    def tour_ordinateur(self):
        ...
        if nav_coule:
            for (lx, ly) in nav_coule.positions:
                self.grille_boutons_joueur[lx][ly].config(bg="purple", text="X")
    ...
    def update_navires_en_vie(self):
        navires_en_vie_joueur1 = len([nav for nav in self.joueur1.navires if not nav.est_coule()])
        navires_en_vie_joueur2 = len([nav for nav in self.joueur2.navires if not nav.est_coule()])

        self.label_nb_navires_joueur1.config(text=f"Navires en vie: {navires_en_vie_joueur1}")
        self.label_nb_navires_joueur2.config(text=f"Navires en vie: {navires_en_vie_joueur2}")

    def update_stats_labels(self):
        self.label_stats_joueur1.config(text=f"Tirs réussis: {self.hits_joueur1} | Tirs manqués: {self.misses_joueur1}")
        self.label_stats_joueur2.config(text=f"Tirs réussis: {self.hits_joueur2} | Tirs manqués: {self.misses_joueur2}")

    def update_time(self):
        elapsed_time = time.time() - self.start_time
        self.label_time.config(text=f"Temps écoulé: {elapsed_time:.2f} s")
    ...
    def nouvelle_partie(self):
        ...
        for i in range(self.joueur1.plateau.taille):
            for j in range(self.joueur1.plateau.taille):
                self.grille_boutons_joueur[i][j].config(state="normal", bg="SystemButtonFace", text=" ")

        for i in range(self.joueur2.plateau.taille):
            for j in range(self.joueur2.plateau.taille):
                self.grille_boutons_ordinateur[i][j].config(state="normal", bg="SystemButtonFace", text=" ")
